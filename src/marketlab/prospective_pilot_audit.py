...
def _scan_secret(directory: Path, secret_value: str | None) -> dict[str, Any]:
    parameter_marker_files: list[str] = []
    ...
    for path in sorted(directory.iterdir()):
        ...
        lowered = data.lower()
        # `response_url_without_api_key` is a legitimate manifest field name.
        # Detect only actual API query/body parameter forms or the exact secret.
        if b"apikey=" in lowered or b'"apikey"' in lowered:
            parameter_marker_files.append(path.name)
        if secret_bytes and secret_bytes in data:
            exact_secret_files.append(path.name)
    return {
        "api_key_parameter_markers": parameter_marker_files,
        "exact_secret_matches": exact_secret_files,
        "passed": not parameter_marker_files and not exact_secret_files,
    }
...
def _group_quote_states(frame: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, int]]:
    ...
    for (event_id, bookmaker_key, market_key), group in frame.groupby(
        ["event_id", "bookmaker_key", "market_key"], sort=True, dropna=False
    ):
        ...
        if (
            ...
            or (prices <= 1.0).any()
            or not group["price_valid_decimal"].map(_truthy).all()
        ):
        ...
        effective_updates = market_updates.dropna()
        if effective_updates.empty:
            effective_updates = bookmaker_updates.dropna()
        rows.append(
            {
                "event_id": event_text,
                ...
                "commence_time": pd.to_datetime(
                    commence_values[0], utc=True, errors="coerce"
                ),
                "effective_update_time": effective_updates.max()
                if not effective_updates.empty
                else pd.NaT,
            }
        )
...
def audit_snapshot_directory(
    ...
    *,
    ...
    market_frame = frame[
        frame["market_key"].astype(str) == policy.required_market
    ].copy()
    ...
    complete_events = (
        int(quote_states["event_id"].nunique()) if not quote_states.empty else 0
    )
    if quote_states.empty:
        ...
        coverage_by_event = quote_states.groupby("event_id")[
            "bookmaker_key"
        ].nunique()
    ...
    covered_events = (
        int((coverage_by_event >= policy.minimum_complete_books_per_event).sum())
    ...
    normalized_diagnostics = manifest.get("diagnostics", {})
    invalid_diagnostics_total = sum(
        int(normalized_diagnostics.get(key, 0) or 0)
        for key in (
            "malformed_events",
    ...
    return {
        ...
        "coverage": {
            ...
            "effective_update_age_seconds_median": float(
                update_age_seconds.median()
            )
...
def audit_snapshot_root(
    ...
    *,
    ...
    if len(directories) != 1:
        raise ValueError(
            "first authenticated pilot must contain exactly one snapshot, "
