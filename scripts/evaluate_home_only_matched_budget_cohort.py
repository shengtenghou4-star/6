...
from evaluate_prospective_matched_budget_cohort import sha256, write_json
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Freeze and evaluate the future-activated home-only cohort."
    )
    ...
    try:
        ...
        if started < campaign_end:
            raise RuntimeError(
                "home-only campaign-close evaluation cannot run before campaign end"
            )

        candidates = pd.read_csv(candidates_path, low_memory=False)
        home_candidates, home_filter = filter_home_candidates(candidates)
        raw, residual, freeze_diagnostics = freeze_campaign_cohort_policies(
            home_candidates, policy
        )
        if not (
            raw["raw_candidate_outcome"].astype(str).eq(EXPECTED_OUTCOME).all()
            and residual["raw_candidate_outcome"].astype(str).eq(EXPECTED_OUTCOME).all()
        ):
            raise RuntimeError("non-home row entered a home-only policy ledger")
        ...
        freeze_manifest = {
            "schema_version": 1,
            "status": "home_only_policies_frozen_before_closing",
            ...
            "home_candidate_input": {
                "rows": int(len(home_candidates)),
            ...
            "research_only": True,
            "no_execution": True,
            "existing_campaign_evaluations_replaced": False,
        }
        ...
        manifest = {
            "schema_version": 1,
            "status": "completed",
            "challenger": "home_only_original_v2_candidates",
