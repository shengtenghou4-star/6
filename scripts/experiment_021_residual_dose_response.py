...
def deterministic_bins(
    ...
    for _, group in frame.groupby(group_columns, sort=True):
        ordered = group.sort_values(
            [score_column, "match_id", "book_slot", "selected_outcome_index"],
            kind="mergesort",
        ).index.to_numpy()
        assigned = np.minimum(np.arange(len(ordered)) * bins // len(ordered), bins - 1)
...
def prepare(frame: pd.DataFrame) -> tuple[pd.DataFrame, list[np.ndarray]]:
    required = {
        "match_id", "match_date", "hours_before_kickoff", "book_slot",
        "selected_outcome_index", "baseline_score", "rank_score",
        "residual_uplift", "observation_decimal_odds", "log_odds_clv",
        "fair_probability_clv", "won",
    }
    ...
    numeric = [
        "baseline_score", "rank_score", "residual_uplift",
        "observation_decimal_odds", "log_odds_clv", "fair_probability_clv",
    ]
    ...
    output["baseline_bin"] = deterministic_bins(
        output, ["hours_before_kickoff"], "baseline_score", BASELINE_BINS
    )
    strata = ["book_slot", "hours_before_kickoff", "baseline_bin"]
    ...
    output["dose_bin"] = deterministic_bins(
        output, strata, "residual_uplift", DOSE_BINS
    )
    ...
    output["unit_return"] = np.where(
        output["won"].astype(bool), output["observation_decimal_odds"] - 1.0, -1.0
    )
...
def circular_shift_null(
    ...
    for start in range(0, replicates, batch_size):
        ...
        clv_num = np.zeros(width)
        return_num = np.zeros(width)
        top_clv = np.zeros(width)
        bottom_clv = np.zeros(width)
        top_return = np.zeros(width)
        bottom_return = np.zeros(width)
        ...
        for indices in groups:
            ...
            shifted_indices = (np.arange(count)[None, :] - offsets[:, None]) % count
            shifted_z = z[indices][shifted_indices]
            shifted_dose = dose[indices][shifted_indices]
            local_clv = clv[indices]
            local_return = returns[indices]
            clv_num += shifted_z @ local_clv
            return_num += shifted_z @ local_return
            top = shifted_dose == 9
            bottom = shifted_dose == 0
            top_clv += top @ local_clv
            bottom_clv += bottom @ local_clv
            top_return += top @ local_return
            bottom_return += bottom @ local_return
            top_count += top.sum(axis=1)
            bottom_count += bottom.sum(axis=1)
        result[start : start + width, 0] = clv_num / denominator
        result[start : start + width, 1] = top_clv / top_count - bottom_clv / bottom_count
        result[start : start + width, 2] = return_num / denominator
        result[start : start + width, 3] = top_return / top_count - bottom_return / bottom_count
    return pd.DataFrame(
        result,
        columns=[
            "clv_slope", "clv_top_minus_bottom",
            "return_slope", "return_top_minus_bottom",
        ],
    )
    ...
    working = pd.DataFrame(
        {
            ...
            "top_n": (dose == 9).astype(np.int8),
            "bottom_sum": np.where(dose == 0, value, 0.0),
            "bottom_n": (dose == 0).astype(np.int8),
        }
    )
...
def main() -> None:
    ...
    dose_summary.to_csv(root / "dose-summary.csv", index=False)
    ...
    cutoff_table.to_csv(root / "cutoff-dose-response.csv", index=False)

    null = circular_shift_null(frame, groups, args.replicates, args.seed)
    null.to_csv(root / "circular-shift-null.csv.gz", index=False, compression="gzip")
    clv = statistic(frame, "log_odds_clv")
    fair = statistic(frame, "fair_probability_clv")
    returns = statistic(frame, "unit_return")
    placebo = {
        "replicates": int(args.replicates),
        "construction": "chronological circular shift within bookmaker x cutoff x baseline-score bin",
        "clv_slope_upper_p": upper_tail_p(null["clv_slope"].to_numpy(float), clv["within_stratum_slope"]),
        "clv_top_minus_bottom_upper_p": upper_tail_p(null["clv_top_minus_bottom"].to_numpy(float), clv["top_minus_bottom"]),
        "return_slope_upper_p": upper_tail_p(null["return_slope"].to_numpy(float), returns["within_stratum_slope"]),
        "return_top_minus_bottom_upper_p": upper_tail_p(null["return_top_minus_bottom"].to_numpy(float), returns["top_minus_bottom"]),
    }
    bootstrap = {
        "closing_log_clv": event_cluster_bootstrap(frame, "log_odds_clv", args.replicates, args.seed + 101),
        "unit_return": event_cluster_bootstrap(frame, "unit_return", args.replicates, args.seed + 202),
    }
    positive_cutoffs = int((cutoff_table["clv_top_minus_bottom"] > 0.0).sum())
